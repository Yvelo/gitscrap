...
def get_random_repo_from_commits(current_repo, tag):
    if current_repo.commits_count > 0:
        ...
        committer = GitUser(committer_login, tag)
        return get_random_repo_from_owner(committer, current_repo, tag)
    else:
        return current_repo
...
def move_to_next_repository(current_repo, probabilities, tag):
    ...
    if probability_level>random_level:
        if current_repo.fork:
            new_repo = GitRepository(current_repo.parent_full_name, tag)
        random_level = 1.0
    ...
    if probability_level>random_level:
        current_owner = GitUser(current_repo.owner_login, tag)
        new_repo = get_random_repo_from_owner(current_owner, current_repo, tag)
        random_level = 1.0
    ...
    if probability_level>random_level:
        current_owner = GitUser(current_repo.owner_login, tag)
        new_repo = get_random_repo_from_followers(current_owner, current_repo, tag)
        random_level = 1.0
    ...
    if probability_level>random_level:
        current_owner = GitUser(current_repo.owner_login, tag)
        new_repo = get_random_repo_from_following(current_owner, current_repo, tag)
        random_level = 1.0
    ...
    if probability_level>random_level:
        new_repo = get_random_repo_from_commits(current_repo, tag)
        random_level = 1.0
